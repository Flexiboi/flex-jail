...
function effect(effectid, amount)
    ...
    elseif string.lower(effectid) == 'alieneffect' then
        AlienEffect()
    elseif string.lower(effectid) == 'armorhealth' then
        ArmorHealth(amount)
    end
end
...
function HealOxy()
    ...
    while count > 0 do
        Wait(1000)
        count = count - 1
        SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) + 9)
    end
    healing = false
end

function ArmorHealth(amount)
    if not healing then
        healing = true
    else
        return
    ...
    local count = amount.count
    TriggerServerEvent('qb-armor:server:SetArmour', count*amount.amount)
    while count > 0 do
        Wait(1000)
        count = count - 1
        SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) + amount.amount)
    end
    healing = false
end
